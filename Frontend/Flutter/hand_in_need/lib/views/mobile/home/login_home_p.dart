...
class _Login_HomeScreenState extends State<Login_HomeScreen>
  ...
  Future<void> GetPostInfo() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        PostInfoList.clear();
        print(
            "Data insert in post info list for home screen failed in home screen.");
        return;
      }
    } catch (obj) {
      PostInfoList.clear();
      print(
          "Exception caught while fetching post info data for home screen in http method");
      print(obj.toString());
      return;
    }
  }
            ...
            return AnimatedContainer(
              ...
              child: orientation == Orientation.portrait
                  ? _buildPortraitLayout(
                      context, shortestval, widthval, heightval)
  ...
  Widget _buildPortraitLayout(BuildContext context, double shortestval,
  ...
  Widget _buildLandscapeLayout(BuildContext context, double shortestval,
      double widthval, double heightval) {
    return _fadeAnimation != null
        ? FadeTransition(
            opacity: _fadeAnimation!,
  ...
  Widget _buildPostCard_User_Home_Screen(
    ...
    return AnimatedContainer(
      ...
      child: isPortrait
          ? _buildPortraitPostContent(
              post, context, shortestval, widthval, heightval)
          : _buildLandscapePostContent(
              post, context, shortestval, widthval, heightval),
    );
  }

  Widget _buildPortraitPostContent(PostInfoModel post, BuildContext context,
  ...
  Widget _buildLandscapePostContent(PostInfoModel post, BuildContext context,
      double shortestval, double widthval, double heightval) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        SizedBox(height: shortestval * 0.02),
        _buildVideoButton(post, context, shortestval, widthval, heightval),
        _buildActionButtons(post, context, shortestval, widthval, heightval,
            isPortrait: false),
      ],
    );
  }
  ...
  Widget _buildHeader(
      PostInfoModel post, BuildContext context, double shortestval) {
    return Padding(
  ...
  Widget _buildVideoButton(PostInfoModel post, BuildContext context,
      double shortestval, double widthval, double heightval) {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: shortestval * 0.03),
      child: Container(
  ...
  Widget _buildActionButtons(PostInfoModel post, BuildContext context,
      double shortestval, double widthval, double heightval,
      {required bool isPortrait}) {
    return Padding(
      padding: EdgeInsets.all(shortestval * 0.03),
          ...
          Expanded(
            child: Obx(
              () => Container(
                width: isPortrait
                    ? widthval * 0.45
                    : widthval * 0.18, // Further reduced width for landscape
                child: CommonButton_loading(
                  ...
                  textStyle: TextStyle(
                    ...
                    fontSize: isPortrait
                        ? shortestval * 0.04
                        : shortestval * 0.03, // Even smaller font in landscape
                    overflow: TextOverflow.ellipsis, // Handle text overflow
                  ),
                  padding: EdgeInsets.symmetric(
                    horizontal:
                        shortestval * 0.01, // Minimal padding in landscape
